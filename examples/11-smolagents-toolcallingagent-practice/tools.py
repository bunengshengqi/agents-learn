...
这个文件是给 ToolCallingAgent 准备的“工具菜单”。

ToolCallingAgent 不直接执行模型生成的 Python Action 代码。
它会让模型生成结构化工具调用，比如：

{"name": "get_weather", "arguments": {"city": "苏州"}}

smolagents 解析这个结构后，再调用这里注册的 Python 函数。
"""
...
def recommend_food(preference: str) -> str:
    ...
    Args:
        preference: 用户偏好关键词，比如减脂、重口味、便宜、省钱、清淡。
    """
    if "减脂" in preference:
...
def convert_and_tax(price: float, exchange_rate: float, tax_rate: float) -> float:
    ...
    Args:
        price: 本地价格。
        exchange_rate: 本地货币兑换美元的汇率。
        tax_rate: 税率，比如 0.1 表示 10%。
    """
...
def estimate_final_price(converted_price: float, shipping_cost: float) -> float:
    ...
    Args:
        converted_price: 换算成美元并加税后的价格。
        shipping_cost: 运费，单位为美元。
    """
...
def get_simple_tools() -> list:
    """
    返回适合 ToolCallingAgent 简单 JSON 工具调用的工具。
    """
    return [
        get_weather,
        get_menu,
...
def get_price_tools() -> list:
    """
    返回价格比较工具，用来展示 ToolCallingAgent 在复杂任务里的局限。
    """
    return [
        lookup_rates,
        lookup_phone_price,
...
def get_all_tools() -> list:
    ...
    统一返回 Day11 的全部工具。
    """
    return [*get_simple_tools(), *get_price_tools()]
